...
it('should render components list', () => {
	const actual = shallow(<StyleGuide codeKey={1} config={config} sections={sections} slots={{}} />);
...
it('should render welcome screen', () => {
	const actual = shallow(
		<StyleGuide codeKey={1} config={config} sections={[]} slots={{}} welcomeScreen />
	);
...
describe('sidebar rendering', () => {
	it('renderer should have sidebar if showSidebar is not set', () => {
		const wrapper = shallow(
			<StyleGuide codeKey={1} config={config} sections={sections} slots={{}} />
		);
	...
	it('renderer should not have sidebar in isolation mode', () => {
		const wrapper = shallow(
			<StyleGuide codeKey={1} config={config} sections={sections} slots={{}} isolatedComponent />
		);
