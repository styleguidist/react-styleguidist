...
export default function(
	...
	const template = isFunction(config.template) ? config.iframeTemplate : MiniHtmlWebpackTemplate;
	const templateContext = isFunction(config.template) ? {} : config.iframeTemplate;
	const htmlPluginOptions = {
		context: {
			...templateContext,
			publicPath: 'iframe/',
			title: config.title, // TODO Title of component / number of example (similar to isolated mode)
			container: 'root',
		},
		template,
	};
	...
	let webpackConfig: Configuration = {
		name: 'iframe',
		entry: [path.resolve(sourceDir, 'iframe'), ...config.require],
		mode: env,
		output: {
			path: `${config.styleguideDir}/iframe`,
	...
	if (isProd) {
		...
		webpackConfig = merge(webpackConfig, {
			output: {
				filename: 'build/iframe/bundle.[chunkhash:8].js',
				chunkFilename: 'build/iframe/[name].[chunkhash:8].js',
			},
			plugins: [
