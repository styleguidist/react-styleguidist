...
const shouldShowDefaultExample = _ref.shouldShowDefaultExample; // Replace placeholders (__COMPONENT__) with the passed-in component name
  ...
  const requireContextCode = b.program( // var name = require(path)
  map(fullContext, (requireRequest, name) => b.variableDeclaration('var', [b.variableDeclarator(b.identifier(name), requireIt(requireRequest).toAST())]))); // Stringify examples object except the evalInContext function
