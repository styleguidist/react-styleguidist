...
export default function getRouteData(sections, hash, pagePerSection) {
  ...
  const infoFromHash = getInfoFromHash(hash, pagePerSection); // Name of the filtered component/section to show isolated (/#!/Button → Button)

  let targetName = infoFromHash.targetName;

      
let hashArray = infoFromHash.hashArray;
  const targetIndex = infoFromHash.targetIndex;

      
const isolate = infoFromHash.isolate;
      ...
      hashArray.forEach(function (hashName, index) {
        ...
        if (filteredSections.length) {
          ...
          if (section) {
            ...
            const isLastHashName = !hashArray[index + 1]; // When sectionDepth is bigger than 0, their children should be filtered
